...
Project: Economic Conditions and Crime Rates in the United States
Authors: Arjun & Kishan
Description: Cleans FBI UCR crime data and USDA ERS unemployment data,
             then integrates them into a unified state-year dataset.
"""
...
# ─────────────────────────────────────────
# 2. CLEAN CRIME DATASET
# ─────────────────────────────────────────

# 2a. Rename columns for readability
...
# 2b. Remove "United States" national aggregate row (not a state)
crime = crime[crime["State"] != "United States"]

# 2c. Filter to overlapping years with unemployment data (2000-2019)
crime = crime[(crime["Year"] >= 2000) & (crime["Year"] <= 2019)].copy()

print(f"Crime after cleaning: {crime.shape} | Missing: {crime.isnull().sum().sum()}")

# ─────────────────────────────────────────
# 3. CLEAN UNEMPLOYMENT DATASET
# ─────────────────────────────────────────

# 3a. Filter to state-level rows (FIPS codes divisible by 1000, excluding 0 = US)
state_unemp = unemp_raw[
    (unemp_raw["FIPS_Code"] % 1000 == 0) &
    (unemp_raw["FIPS_Code"] != 0)
].copy()

# 3b. Parse year and metric name from the Attribute column
...
# 3c. Pivot long -> wide so each row is (State, Year)
...
# 3d. Filter to overlapping years
unemp_wide = unemp_wide[(unemp_wide["Year"] >= 2000) & (unemp_wide["Year"] <= 2019)].copy()

# 3e. Map state abbreviations to full names for joining
state_abbrev = {
...
# Select only the unemployment columns we need for analysis
